...
step1: 
  g++ Tree-BML-main.cpp -O3 -std=c++11 -funroll-all-loops -march=native -I"../tfhe/src/include/" -L"../tfhe/build/libtfhe/" -ltfhe-spqlios-fma -lm -g -o TreeBMLmain
step2: 
  ./TreeBMLmain 2
*/
...
void test_LUT_6_bit_to_6_bit_BML(fbt_integer * output, fbt_integer * input, fbt_context_BML * context, const int theta){
  ...
  /*
    if f_l1b[5] == 0, 
      carry out f_l1b[3] times bootstrappings with theta
    else{
      carry out f_l1b[3] - 1 times bootstrappings with theta
      carry out 1 times bootstrapping with theta = f_lb[4]
    } 
  ...
  */      
  ...
  fbt_main_loop_BML(function, 10, input->lwe_samples, context, theta);
...
void test_LUT_8_bit_to_8_bit_BML(fbt_integer * output, fbt_integer * input, fbt_context_BML * context, const int theta){
  ...
  int f_l0b[6] = {fbt_op_f_bootstrap_BML, 0, 0, 1, theta, 0};
  // theta = 2

  if (theta == 1)
  {       
    f_l0b[3] = 2; f_l0b[4] = theta; f_l0b[5] = 0;
  }
  else if(theta == 3)
  {
...
void test_LUT_12_bit_to_12_bit_BML(fbt_integer * output, fbt_integer * input, fbt_context_BML * context, const int theta){
  ...
  if (theta == 1)
  {       
    f_l0b[3] = 3; f_l0b[4] = theta; f_l0b[5] = 0;
  }
  else if(theta == 3)
  {
    f_l0b[3] = 1; f_l0b[4] = theta; f_l0b[5] = 1;
  ...
  fbt_main_loop_BML(function, 19, input->lwe_samples, context, theta);
...
/**/
void test_32_bit_integer_comparison_new_ks(LweSample * output, fbt_integer * input1, fbt_integer * input2, fbt_context_BML * context, const int theta){
  const TFheGateBootstrappingParameterSet * params = context->params;
  ...
  fbt_main_loop_BML(function, 64, context->lwe_temp_in, context, theta);

  lweCopy(output, &context->output[0], &params->tgsw_params->tlwe_params->extracted_lweparams);
}

// relu
void relu_8bit_BML(fbt_integer * output, fbt_integer * input, fbt_context_BML * context, int theta){
  const TFheGateBootstrappingParameterSet * params = context->params;
  ...
  fbt_main_loop_BML(function, 2, context->lwe_temp_in, context, theta);
...
int main(int argc, char const *argv[])
{
  ...
  TFheFunctionalBootstrappingSecretKeySet_BML* key_BML = new_random_functional_bootstrapping_secret_keyset_BML(params, 2, theta);
 ...
 /*Relu*/
  ...
  for (size_t i = 0; i < NUM_EXE; i++)
    {
      relu_8bit_BML(output_8bit, number1_8bit, context_BML, theta);
    }
  );
...
/* 32-bit integer comparison */
  ...
  for (size_t i = 0; i < NUM_EXE; i++)
    {
      test_32_bit_integer_comparison_new_ks(output, number1_32bit, number2_32bit, context_BML, theta);
    }
  );
